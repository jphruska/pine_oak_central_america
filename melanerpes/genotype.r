...
# make sure reference is indexed with bwa and samtools before use, and use CreateSequenceDictionary in GATK 
	...
	project_directory <- "/lustre/scratch/johruska/central_america_pine_oak/melanerpes"
	directory_name <- "melanerpes_genotype"
	reference_genome_location <- "/lustre/scratch/johruska/colaptes/ref_genome/c05_colaptes_arcs3_edited.fa"
	queue <- "omni"
	cluster <- "quanah"
	output_name <- "melanerpes_genotype"
	popmap <- "melanerpes_popmap.txt"
	individuals <- read.table(popmap, sep="\t")

	# input some items 
	manual_identification <- FALSE # whether using a fai index or manually inputting chromosomes
	genome_index <- "c05_colaptes_arcs3_edited.fa.fai" # for use without manual input
	...
	for(a in 1:length(chr_list_to_genotype)) {
		...
		write(paste("#$ -t 11:15", sep=""), file=a.script, append=T)
