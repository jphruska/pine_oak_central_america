...
# make sure reference is indexed with bwa and samtools before use, and use CreateSequenceDictionary in GATK 
	...
	project_directory <- "/lustre/scratch/johruska/central_america_pine_oak/geothlypis"
	directory_name <- "geothlypis_genotype"
	reference_genome_location <- "/lustre/scratch/johruska/central_america_pine_oak/passer_ref/GCA_003957565.2_bTaeGut1_v1.p_genomic.fasta"
	queue <- "omni"
	cluster <- "quanah"
	output_name <- "geothlypis_genotype"
	popmap <- "geothlypis_popmap.txt"
	individuals <- read.table(popmap, sep="\t")

	# input some items 
	manual_identification <- FALSE # whether using a fai index or manually inputting chromosomes
	genome_index <- "GCA_003957565.2_bTaeGut1_v1.p_genomic.fasta.fai" # for use without manual input
	...
	for(a in 1:length(chr_list_to_genotype)) {
		...
		write(paste("#$ -t 6:10", sep=""), file=a.script, append=T)
